...
const fanControl = handleActions(
    {
        ...
        [CHANGE_WORK]: (state, action) => ({
            ...state,
            work: !state.work,
            status: state.work === true ? '환풍기가 작동하고 있지 않아요' : '환풍기가 작동하고 있어요',
        ...
        [CHANGE_AUTOWORK]: (state, action) => ({
            ...
            status: state.autoWork === true ? '환풍기가 작동하고 있지 않아요' : `${state.autoWorkPeriod}${state.autoWorkPeriodUnit} 이후에 ${state.autoWorkTime}${state.autoWorkTimeUnit} 동안 작동해요`,
            workButtonText: '작동하기'
        }),
